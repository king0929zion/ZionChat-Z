...
- 继续优化模型管理页 UI：
  - 模型卡片现在只显示 model name（显示名称），不再显示 model id
  - 模型卡片图标改为使用对应模型的专属图标（AutoAIIcon）
  - 新增顶部刷新区域，显示模型可用性状态并提供刷新按钮
  - 删除按钮改为左滑模型卡片时出现红色圆形按钮的设计

## v0.0.85

- 简化供应商编辑页和模型管理页 UI：
  - 供应商编辑页的 models 入口改为简洁的单行设计，左侧显示机器人图标，右侧文字为 "models"
  - 模型管理页移除供应商信息卡片和刷新按钮
  - 添加模型按钮移至右上角，显示为加号图标
  - 模型卡片简化设计：移除滑动手势和 OutlinedCard 边框，改为点击卡片进入编辑，右侧显示删除按钮

## v0.0.84

- 优化 `设置 -> Bots` 列表页顶部留白：移除多余的 `windowInsetsPadding`，使顶部间距与其他设置页面保持一致。

## v0.0.83

- `设置 -> Bots` 继续按 `ZionChat` 源码样式收口：Bot 详情页改成和 `AddBotScreen` 同节奏的单页编辑结构，页面只保留头像、名称、默认模型和提示词四块核心内容，灰阶、圆角和间距全面对齐现有 Zion 系设置页。
- 修正 `ZPhone -> X` 的 bot 自动互动链路：现在会排除所有内置默认助手，只使用你真正创建的 Bots 参与自动发帖、自动回复和转帖评论；同时移除把系统提示词首行直接拼进兜底文案的逻辑，避免再出现 `You are a helpful assistant...` 这类虚假回复。
